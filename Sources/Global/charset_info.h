#ifndef Retoccilus_Global_Charset_Info
#define Retoccilus_Global_Charset_Info
...
namespace Retoccilus {
    ...
    public:
        explicit CharsetInfoManager(const QString &localeName) : QObject(), mLocaleName(localeName) {
        ...
        QStringList languageNames() {
            ...
            QStringList lst;
                    foreach (const QString &s, languages)lst.append(s);
        ...
        QList<PCharsetInfo> findCharsetsByLanguageName(const QString &languageName) {
            ...
            sort(result.begin(), result.end(), [](const PCharsetInfo &info1, const PCharsetInfo &info2) {
        ...
        QList<PCharsetInfo> findCharsetByLocale(const QString &localeName) {
            QList<PCharsetInfo> result;
                    foreach (const PCharsetInfo &info, mCodePages) {
                    if (info->enabled && info->localeName == localeName) result.append(info);
                }
            return result;
        }
        ...
        QString findLanguageByCharsetName(const QString &encodingName) {
                    foreach (const PCharsetInfo &info, mCodePages) {
                    if (info->enabled && QString::compare(info->name, encodingName, Qt::CaseInsensitive) == 0) {
        ...
        [[nodiscard]] const QString &localeName() {
            return mLocaleName;
        }
